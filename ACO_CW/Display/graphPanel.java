...
public class graphPanel extends JFrame {
    ...
    */
    ...
    private void setScale () {
        ...
        xScale = ((double) maxWidth) / ((double)WIDTH- OFFSET);
        yScale = ((double) maxHeight) / ((double)HEIGHT- OFFSET);
    }
    ...
    private class Panel extends JPanel {
        ...
        private void drawChromosome(Graphics2D graphics) {
            ...
            for (int i = 1; i < array.length; i++) {
                int x1 = (int)(array[i-1].getX() / xScale + OFFSET / 2);
                int y1 = (int)(array[i-1].getY() / yScale + OFFSET / 2);
                int x2 = (int)(array[i].getX() / xScale + OFFSET / 2);
                int y2 = (int)(array[i].getY() / yScale + OFFSET / 2);
        ...
        private void drawCity(Graphics2D graphics) {
            ...
            for (Node node : cityNodes) {
                int x = (int)((node.getX()) / xScale - NODE_SIZE /2 + OFFSET / 2);
                int y = (int)((node.getY()) / yScale - NODE_SIZE /2 + OFFSET / 2);
